 ...
 */
@Configuration
public class LogPacketPublishExecutorConfig {

    @Value("${executor.logPacketPublish.corePoolSize:8}")
    private int corePoolSize;

    @Value("${executor.logPacketPublish.maxPoolSize:16}")
    private int maxPoolSize;

    @Value("${executor.logPacketPublish.queueSize:2000}")
    private int queueSize;

    @Value("${executor.logPacketPublish.keepAliveSeconds:60}")
    private long keepAliveSeconds;

    @Bean(name = "logPacketPublishPool")
    public ExecutorService logPacketPublishPool() {
        ...
        return new ThreadPoolExecutor(
                ...
                r -> {
                    Thread t = new Thread(r, "loggkamel-log-packet-publish-" + Thread.currentThread().getName());
                    t.setDaemon(false);
                    return t;
                },
                rejectionHandler
