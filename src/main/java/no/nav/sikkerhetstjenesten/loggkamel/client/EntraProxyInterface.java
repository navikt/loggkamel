...
public interface EntraProxyInterface {
    ...
    EntraProxyAnsatt getAnsattMedNavIdent(@RequestHeader(AUTHORIZATION) String bearerToken, @PathVariable("navIdent") String navIdent);

    @GetExchange("/api/v1/ansatt/tident/{tIdent}")
    EntraProxyAnsatt getAnsattMedTIdent(@RequestHeader(AUTHORIZATION) String bearerToken, @PathVariable String tIdent);
}
