        ...
        if (logPacketConsumerUri.startsWith("google-storage://")) {
            onCompletion()
                    ...
                    .process(exchange -> {
                        ...
                        log.info("Exchange in headers upon attempting to flush logs: {}", exchange.getIn().getHeaders());
                        log.info("Exchange message headers upon attempting to flush logs: {}", exchange.getMessage().getHeaders());
