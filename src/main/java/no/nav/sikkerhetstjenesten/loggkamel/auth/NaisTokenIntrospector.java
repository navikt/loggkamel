...
public class NaisTokenIntrospector implements OpaqueTokenIntrospector {
    ...
    private static final String TOKEN_INTROSPECTION_ENDPOINT = System.getenv("NAIS_TOKEN_INTROSPECTION_ENDPOINT");
    private static final List<GrantedAuthority> grantedAuthorities = List.of(new SimpleGrantedAuthority("AUTHENTICATED_NAIS_SERVICE"));
    ...
    @Autowired
    public NaisTokenIntrospector(ObjectMapper objectMapper) {
    ...
    public @NonNull OAuth2AuthenticatedPrincipal introspect(@NonNull String token) {
        if (TOKEN_INTROSPECTION_ENDPOINT == null || TOKEN_INTROSPECTION_ENDPOINT.isBlank()) {
        ...
        EntraAuthenticationResponse authenticationResponse = restClient.post()
                .uri(TOKEN_INTROSPECTION_ENDPOINT)
